...
export default function SettingsPage() {
  ...
  async function handleUpdateEmail(e: React.FormEvent) {
    ...
    else { setEmailSuccess(true); setTimeout(() => setEmailSuccess(false), 3000) }
  ...
  async function handleUpdatePassword(e: React.FormEvent) {
    ...
    if (newPassword.length < 6) { setPasswordError('Le mot de passe doit faire au moins 6 caracteres'); return }
  ...
  async function handleDeleteAccount() {
    ...
    if (!res.ok) { setDeleteError(data.error); setDeleting(false); setShowConfirmDelete(false); return }
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="mb-6">
        <h1 className="text-xl md:text-2xl font-semibold text-gray-900">Parametres</h1>
        <p className="text-gray-400 text-xs md:text-sm mt-1">Gere ton compte et tes preferences</p>
      </div>
        ...
        <div className="space-y-4 md:space-y-6">
            ...
            <form onSubmit={handleUpdateEmail} className="space-y-3">
              ...
              {emailSuccess && (
                <div className="flex items-center gap-2 text-green-600 text-xs bg-green-50 px-3 py-2 rounded-lg">
                  <CheckCircle size={13} /><span>Email de confirmation envoye</span>
                </div>
              )}
              <button type="submit" disabled={savingEmail || email === user?.email} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 transition-colors">
                <Save size={14} />{savingEmail ? 'Sauvegarde...' : 'Mettre a jour'}
              </button>
            </form>
          </div>
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5">
            ...
            <form onSubmit={handleUpdatePassword} className="space-y-3">
              ...
              {passwordSuccess && (
                <div className="flex items-center gap-2 text-green-600 text-xs bg-green-50 px-3 py-2 rounded-lg">
                  <CheckCircle size={13} /><span>Mot de passe mis a jour</span>
                </div>
              )}
          ...
          <div className="bg-white border border-red-100 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">Actions irreversibles</p>
              </div>
            </div>
            <div className="bg-red-50 rounded-xl p-3 mb-4">
              <p className="text-xs text-red-600 font-medium mb-1">Supprimer mon compte</p>
              <p className="text-xs text-red-400">Toutes tes donnees seront definitivement supprimees. Cette action est irreversible.</p>
            </div>
            {deleteError && <p className="text-red-500 text-xs bg-red-50 px-3 py-2 rounded-lg mb-3">{deleteError}</p>}
            <button
              type="button"
      ...
      {showConfirmDelete && (
        <ConfirmModal
          ...
          message="Toutes tes donnees seront definitivement supprimees. Candidatures, documents, entretiens et entreprises seront perdus. Cette action est irreversible."
