  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <label className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer ${uploading ? 'bg-gray-100 text-gray-400' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>
          <Upload size={15} />
      ...
      <div className="flex gap-2 mb-5 overflow-x-auto pb-1">
        {([
          { key: 'all',          label: 'Tous'    },
          { key: 'cv',           label: 'CV'      },
          { key: 'cover_letter', label: 'Lettres' },
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-gray-400 text-xs mt-1">Ajoute ton premier document</p>
        </div>
      ) : (
        <>
          {/* Mobile */}
          <div className="md:hidden space-y-2">
            {filtered.map((doc, i) => {
              ...
              return (
                <div
                  key={doc.id}
                  className="bg-white border border-gray-100 rounded-xl p-4 flex items-center gap-3 cursor-pointer hover:border-indigo-200 hover:shadow-sm transition-all active:scale-[0.99]"
          ...
          <div className="hidden md:block bg-white border border-gray-100 rounded-xl overflow-hidden">
            ...
            {filtered.map((doc, i) => {
              ...
              return (
                <div
                  key={doc.id}
                  className="grid grid-cols-12 gap-4 px-5 py-4 border-b border-gray-50 hover:bg-gray-50 transition-colors group items-center cursor-pointer"
                  onClick={() => setViewerIndex(i)}
                >
